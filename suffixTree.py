        ...
        while(self.remainingcount>0):
            if(self.activelength == 0):
                if(self.activenode.children.get(self.string[pos]) is not None):
                    ...
                    self.activenode.children[self.string[pos]]=self.creat_node(pos, leafEnd, isleaf=True)
                    self.remainingcount-=1
            else:
                try:
                    ...
                    if (nextchar == self.string[pos]):
                        ...
                        new_internalnode.children[self.string[pos]] = self.creat_node(pos, leafEnd,isleaf=True) #it is the string[pos]
                    ...
                    node=self.selectnode()
                    node.children[self.string[pos]]=self.creat_node(pos,leafEnd,isleaf=True)
                    if (lastCreatedInternalNode is not None):
